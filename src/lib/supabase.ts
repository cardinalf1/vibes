...
// Default Supabase project URL & verified public anon JWT key for Isha Vibes
...
const rawEnvUrl = (import.meta.env.VITE_SUPABASE_URL as string) || '';
const rawEnvKey = (import.meta.env.VITE_SUPABASE_ANON_KEY as string) || '';

const envUrl = rawEnvUrl.includes('supabase.co') ? rawEnvUrl : DEFAULT_URL;
const envKey = rawEnvKey.startsWith('eyJ') ? rawEnvKey : DEFAULT_KEY;
...
// Purge any stale non-JWT publishable keys or invalid configs from previous attempts
